    ...
    function AutoModeChip(props) {
      ...
      return React.createElement('div', { className: 'dsam_row' },
        React.createElement('span', {
          className: cls,
          title: state === 'on'
            ? 'Auto Mode 已开启：授权请求自动放行（输入 /auto 关闭）'
            : 'Auto Mode 已关闭：授权请求需要确认（输入 /auto 开启）',
          'aria-label': label
        }, label))
    }
