...
export function apply(ctx, config) {
  ...
  // dangerous shell deletes, which fall through so the user is asked even
  // while Auto Mode is on.
  ctx.on('approval/request', (req, next) => {
    if (!isEnabled()) return next()
    if (!isDangerEscalation(req)) return 'allowed-once'
    // Dangerous-delete guard applies to shell tools only (pwsh/bash): their
    // command text can carry the Windows accident shapes (recursive, wildcard,
    // drive-root, trailing-backslash-quote, junction). Filesystem tools
    // (edit/write/read/fs-*) take structured path arguments, not command
