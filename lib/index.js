  ...
  ctx.commands.register({
    ...
    handler: async (invocation) => {
      ...
      invocation.agent.inject(createUserMessage({
        content: [{ type: 'text', text: `Auto Mode is now ${label} (changed by the user).` }],
        // Required by the UserMessage contract: the turn machinery reads
        // message.source.kind while draining the inbox (crash without it).
        source: { kind: 'plugin', plugin: 'auto-mode' },
      }))
      return { kind: 'success', text: `Auto Mode is now ${label}.` }
    },
  })
