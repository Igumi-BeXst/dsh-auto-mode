...
**真实命令文本**,并与 Windows 事故形态匹配——递归删除(`Remove-Item -Recurse`、
`rm -rf`、`rd /s`)、通配符删除、盘符根删除、尾部反斜杠引号路径 bug(经典的
"删除一个链接却扩大到链接目标/根目录"形态)、以及针对 junction/符号链接的删除。
匹配到的命令会穿透到交互应答器,浏览器必然先询问你。文件系统工具(edit/write/
read/fs-*)使用结构化路径参数而非命令字符串,其升级在 Auto Mode 下**始终**自动
放行。模型提示词也写明了同一条规则:破坏性的 shell 删除会弹窗审批,其他需要
全权限的操作会被自动批准。

## 安全提醒

Auto Mode 会放行**所有**请求,包括破坏性操作。运行时上下文会警告模型对不可逆或
高成本操作格外谨慎,并且**破坏性的 shell 删除**——递归、通配符、盘符根或针对
junction/符号链接的删除——即使 Auto Mode 开启也**始终**需要你明确批准。
文件系统工具不会弹窗。任何时刻点击芯片即可关闭。
